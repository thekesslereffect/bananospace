...
const Header = () => {
  ...
  const toggleMenu = () => {
    ...
    return (
        <div className="flex flex-col w-full items-center z-50">
            {/* Desktop View */}
            <div className="hidden md:flex mb-16 mx-auto justify-between items-stretch px-10 max-w-none w-full">
                <Link href={'/'} className={` flex flex-row  text-neutral-700 text-lg items-center`}>
                  <Image src={'/images/banano-mark.png'} width={"40"} height={"40"} alt={"bslogo"} className=" mr-4"/>
                  banano.space
                {/* ▲ banano.space */}
                {/* .space */}
                {/* × */}
                ...
                <Squircle cornerRadius={14}	cornerSmoothing={1} className="flex w-max h-full hover:-translate-y-1 hover:scale-105 duration-300 border-2 border-white/50 backdrop-blur-sm bg-[#ffffff30]  shadow-[inset_0_20px_40px_-30px_rgba(255,255,255,.5)] ">
                      <ConnectWallet
                        btnTitle={"Connect"}
                        modalSize={"compact"}
                        welcomeScreen={{ title: "" }}
          ...
          <div 
            ...
            >
                <Squircle cornerRadius={32}	cornerSmoothing={0.8} className="flex flex-col items-center text-2xl text-neutral-700 duration-300 max-h-[400px] min-h-[400px] border-2 border-white/50 backdrop-blur-sm bg-[#ffffff70]  p-8   col-span-4 shadow-[inset_0_20px_40px_-30px_rgba(255,255,255,.5)] ">
                {/* <div className="flex flex-col text-neutral-700 bg-white border shadow-2xl rounded-t-3xl p-4 items-center"> */}
                    ...
                    <Squircle
                        cornerRadius={20}
                        cornerSmoothing={1}
                        className={`flex w-full justify-center text-3xl px-3  
